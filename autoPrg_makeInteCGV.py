import module_CGVMovieComparator
...
inte_cgv_movie_info = db_ttolae.inte_cgv_movie_info
...
for i in range(0,cgv_cursor.count()) :
    ...
    res = module_CGVMovieComparator.compare(jin, current_document, "inte_title", "director")
...
##
        ...
        dictTemp = {'_id': '', 'movie_id': [], 'inte_title': '', 'movie_title': '', 'director': '', 'review_cnt': [],
                    'gender_ratio': [{'site': 'cgv', 'male': 0.0}],
                    'age': [{'site': 'cgv', 'teen': 0.0, 'twenty': 0.0, 'thirty': 0.0, 'forty': 0.0}], 'score': [], 'nation': '', 'open_date': '', 'genre': ''}
        current_document['_id'] = str(current_document['_id'])
