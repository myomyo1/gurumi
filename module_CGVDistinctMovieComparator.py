...
def compare( collection , compareDict, fieldName,fieldName1) :
    ...
    if fieldName not in compareDict:
        print("code : 3")
        return 3
    find = collection.find({fieldName: compareDict[fieldName]})
    findCnt = collection.find({fieldName: compareDict[fieldName]}).count()
...
def concat(stdDict,compareDict) :
    ...
    dictTemp['gender_ratio'][0]['male'] = compareDict['gender_ratio'][0]["male"]
    dictTemp['age'][0]['teen'] = compareDict['age'][0]['teen']
    dictTemp['age'][0]['twenty'] = compareDict['age'][0]['twenty']
    dictTemp['age'][0]['thirty'] = compareDict['age'][0]['thirty']
    dictTemp['age'][0]['forty'] = compareDict['age'][0]['forty']
    return dictTemp
